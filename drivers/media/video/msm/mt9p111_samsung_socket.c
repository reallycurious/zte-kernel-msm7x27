...
static long mt9p111_reg_init(void)
{
    long rc;
    uint32_t i;
    uint16_t is_otp_written;
    uint16_t otp_check_addr;
    ...
    /*
     * initialize the OTP memory of lens shading
     */
    rc = mt9p111_i2c_write(mt9p111_client->addr, 0x098E, 0x602A, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    }
    ...
    /*
      * Check whether OTP is available for reading
      */
    is_otp_written = 0;
    rc = mt9p111_i2c_read(mt9p111_client->addr, 0xE023, &is_otp_written, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    }
    ...
    /*
      * is_otp_written: 6-bit of 1st byte in reg of 0xE023
      * = 0x4000: OTP is available for reading
      * = others: OTP is unavailable for reading
      */
    i = 0;
    do
    {
        ...
        /* ZTE_JIA_CAM_20100930
         * add time delay for OTP read sequence, from 100ms to 200ms
         */
        mdelay(200);

        CDBG("%s: check otp memory is whether available for reading, is_otp_written=0x%x!\n",
             __func__, is_otp_written);
        
        i++;
    } while ((i < 10) && ((0x4000 & is_otp_written) == 0));
    
    CDBG("%s: otp is available for reading, otp_avail_flag=0x%x !!!!\n", __func__, is_otp_written);

    if((is_otp_written & 0x4000) == 0)
    {
    	CCRT("%s: otp is not available for reading!\n", __func__);
        return -EIO;
    }
	
    /*
      * Check whether OTP has been writen now
      */
    rc = mt9p111_i2c_write(mt9p111_client->addr, 0x098E, 0x6020, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    }
    ...
    /*
     * Check 3nd OTP memory
     */
    otp_check_addr = 0x02DE;
    rc = mt9p111_i2c_write(mt9p111_client->addr, 0xE024, otp_check_addr, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    }

    rc = mt9p111_i2c_write(mt9p111_client->addr, 0xE02A, 0xA010, WORD_LEN);
    if (rc < 0)
    {
        return rc;
    }
    ...
    do
    {
        rc = mt9p111_i2c_read(mt9p111_client->addr, 0xE026, &is_otp_written, WORD_LEN);
        if (rc < 0)
        {
            return rc;
        }
        ...
        CDBG("%s: check for the 3rd otp memory, is_otp_written =0x%x !\n", __func__, is_otp_written); 
        
        i++;
    } while ((i < 10) && ((0x0000 | is_otp_written) == 0));

    /*
     * Check other two OTP memory
     */
    if (is_otp_written == 0x0) 
    {
        /*
          * Check 2nd OTP memory
          */
      	otp_check_addr = 0x01DE;
	    rc = mt9p111_i2c_write(mt9p111_client->addr, 0xE024, otp_check_addr, WORD_LEN);
	    if (rc < 0)
	    {
	        return rc;
        ...
        do
        {
            ...
            CDBG("%s: check for the 2nd otp memory, is_otp_written =0x%x !\n", __func__, is_otp_written); 
        ...
        {
            /*
               * Check 1st OTP memory
               */
            otp_check_addr = 0x00DE;
            rc = mt9p111_i2c_write(mt9p111_client->addr, 0xE024, 0x00CA, WORD_LEN);
            if (rc < 0)
            {
            return rc;
            }
            ...
            do
            {
                ...
                CDBG("%s: check for the 1st otp memory, is_otp_written =0x%x !\n", __func__, is_otp_written); 
			...
			if(is_otp_written == 0x0)
			{
				CCRT("%s: all the otp memory is not written,just go to reg_init_tbl !\n", __func__);
				return -EIO;
			}
        }
    ...
    CDBG("%s: otp memory is written, is_otp_written=0x%x, otp_check_addr=0x%x\n",
         __func__, is_otp_written,otp_check_addr);

    /*
      * Load OTP memory, in which LSC is written, to sensor's working memory
      */
    switch (otp_check_addr)
    {
        case 0x02DE:
        {
            rc = mt9p111_i2c_write_table(mt9p111_regs.otp_lsc_reg_tbl[2],
                                         mt9p111_regs.otp_lsc_reg_tbl_sz[2]);
    	    if (rc < 0)
    	    {
    	        return rc;
    	    }
        ...
        case 0x01DE:
        {
            rc = mt9p111_i2c_write_table(mt9p111_regs.otp_lsc_reg_tbl[1],
                                         mt9p111_regs.otp_lsc_reg_tbl_sz[1]);
    	    if (rc < 0)
    	    {
    	        return rc;
    	    }
        ...
        case 0x00DE:
        {
            rc = mt9p111_i2c_write_table(mt9p111_regs.otp_lsc_reg_tbl[0],
                                         mt9p111_regs.otp_lsc_reg_tbl_sz[0]);
    	    if (rc < 0)
    	    {
    	        return rc;
    	    }
        ...
        default:
            CCRT(" All of otp is not written!\n");
            return -EIO;
    }
...
static int mt9p111_sensor_release_internal(void)
{
    ...
    /*
     * Solve sensor current leakage issue.
     * Write this reg to shut off all sensor gpios.
     * by lijing LIJING_CAM_20100621
     */ 
